...
router.get('/', (request, response) => {
  ...
  if (User.find(api_key) === api_key) {
    fetchCoords = async () => {
      const key = await yaml.safeLoad(fs.readFileSync("./config.yml", "utf8"))["google_geocoding_api"];

      try {
        let response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${location}&key=${key}`);
    ...
    fetchForecastData = async () => {
      const key = await yaml.safeLoad(fs.readFileSync("./config.yml", "utf8"))["dark_sky_api"];

      var lat = await fetchCoords().then(resp => resp["lat"]);
      var lng = await fetchCoords().then(resp => resp["lng"]);

      try {
        let response = await fetch(`https://api.darksky.net/forecast/${key}/${lat},${lng}`);
    ...
    (formatData = async () => {
      let rawData = await fetchForecastData().then(resp => new Forecast(location, resp))
